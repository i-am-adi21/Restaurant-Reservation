	...
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		if (requestCode == 0) {
			if (resultCode == RESULT_OK) {
				...
				Toast.makeText(getApplicationContext(),"hellooo"+ViewBookings.table_ids,Toast.LENGTH_LONG).show();

				if(contents.equalsIgnoreCase(ViewBookings.table_ids))
				{
					Toast.makeText(getApplicationContext(),"hiiiiiiiiiii",Toast.LENGTH_LONG).show();

					JsonReq jr= new JsonReq();
					jr.json_response=(JsonResponse) AndroidBarcodeQrExample.this;
					String q="view_qr_code?reservation_ids="+ViewBookings.reservation_ids;
					q.replace("", "%20");
					jr.execute(q);
				}
				else{
